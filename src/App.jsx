...
import { Archive, ArrowDownToLine, BookOpen, CalendarDays, ChevronDown, ChevronRight, CircleHelp, Columns3, Database, FileText, Folder, Grid2X2, Hash, LayoutGrid, List, Menu, Pin, Plus, RefreshCw, Search, SlidersHorizontal, Sparkles, Trash2, X, Table2, Wrench } from 'lucide-react';
...
import { filterNotes, localDay, statuses, boardStatuses, priorities } from './utils.js';
import { toVisual } from './rich-content.js';
import SyncStatus from './SyncStatus.jsx';
import SolutionsView from './Solutions.jsx';
...
const names = { all: 'Todas las notas', pinned: 'Notas fijadas', kanban: 'Tablero kanban', today: 'Mi día', archive: 'Archivadas', trash: 'Papelera', category: 'Categoría', solutions: 'Sección de Soluciones' };
const descriptions = { all: 'Todo lo que necesitas recordar, en un mismo lugar.', pinned: 'Tus ideas importantes, siempre a mano.', kanban: 'Dale un siguiente paso a cada idea.', today: 'Un poco de enfoque para lo que viene hoy.', archive: 'Un espacio para lo que quieres conservar.', trash: 'Las notas eliminadas se conservan aquí. Puedes restaurarlas.', category: 'Cada tema tiene su propio espacio.', solutions: 'Base operativa de procedimientos SQL, scripts de soporte y resolución de incidencias para Solución Médica, Tablet, MDS, SAP y Programación Médica.' };
...
export default function App() {
  ...
  const refresh = useCallback(async () => { try { const data = await api('/workspace'); setData(data); setConnected(data.sync ? data.sync.initialized : true); setError(''); } catch (e) { setConnected(false); setError(e.message); throw e; } finally { setLoading(false); } }, []);
  useEffect(() => { refresh().catch(() => {});  }, [refresh]);
  useEffect(() => {
    if (!data.sync || editor || modal || actionBusy || pendingIds.size) return;
    const timer = setInterval(() => refresh().catch(() => {}), 5000);
    return () => clearInterval(timer);
  }, [Boolean(data.sync), editor, modal, actionBusy, pendingIds.size, refresh]);
  useEffect(() => { setPage(1); }, [scope, category, query, status, priority, sort, view]);
  const newNote = useCallback((state = currentView === 'board' ? 'todo' : 'inbox', template = '', dueDate = null) => {
    ...
    if (template === 'solution') { draft.title = 'Ticket / Solución TI'; draft.content = '## Descripción del Problema\n\n\n## Entorno y Base de Datos\n- **Sistema:** Solución Médica / Tablet / MDS\n- **Base de Datos:** hipocrates / BD_Sanna_ambulatoria\n\n## Consulta / Script SQL\n```sql\n-- Consulta ejecutada\n\n```\n\n## Resultado / Validación\n\n'; draft.tags = ['solucion', 'ti']; }
    if (template === 'sql' || template === 'solution') draft.format = 'markdown';
    else if (draft.content) draft.content = toVisual(draft.content);
    setEditorHistory(false); setEditor(draft); setMobile(false);
  }, [category, data.categories, currentView]);
  useEffect(() => {
  ...
  function changeView(v) { setView(v); setPref('apuntes.view', v); if (v === 'board' && status === 'inbox') setStatus(''); }
  function upsert(note) { setData(d => ({ ...d, sync: d.sync ? { ...d.sync, pending: d.sync.pending + 1 } : undefined, notes: d.notes.some(n => n.id === note.id) ? d.notes.map(n => n.id === note.id ? note : n) : [note, ...d.notes] })); setConnected(true); }
  ...
  const visible = useMemo(() => filterNotes(data.notes, { scope, view: currentView, query, category, status, priority, sort }), [data.notes, scope, currentView, query, category, status, priority, sort]);
  ...
  const navItems = [{ id: 'all', icon: FileText, label: 'Todas las notas', count: active.length }, { id: 'today', icon: CalendarDays, label: 'Mi día', count: today.length }, { id: 'pinned', icon: Pin, label: 'Fijadas', count: pinned.length }, { id: 'kanban', icon: Columns3, label: 'Tablero kanban', badge: 'NUEVO' }, { id: 'solutions', icon: Wrench, label: 'Soluciones TI', badge: 'PRO' }];
  ...
  return <div className="app-shell">{mobile && <div className="sidebar-overlay" onClick={() => setMobile(false)}/>}
    <aside className={`sidebar ${mobile ? 'open' : ''}`}><a className="brand" href="#" onClick={e => { e.preventDefault(); navigate('all'); }}><img src="/favicon.svg" alt=""/><span>apuntes<span className="brand-dot">.</span></span></a>
      ...
      <div className="sidebar-bottom"><nav><button className={`nav-item ${scope === 'archive' ? 'active' : ''}`} onClick={() => navigate('archive')}><Archive size={17}/><span>Archivadas</span><span className="nav-count">{archived.length}</span></button><button className={`nav-item ${scope === 'trash' ? 'active' : ''}`} onClick={() => navigate('trash')}><Trash2 size={17}/><span>Papelera</span><span className="nav-count">{trashed.length}</span></button><button className="nav-item" onClick={() => setModal('backup')} disabled={!connected}><ArrowDownToLine size={17}/><span>Importar / exportar</span></button></nav><div className="storage-card"><span className={`connection-dot ${connected ? '' : 'offline'}`}/><div><strong>{connected ? data.sync ? 'Guardado en este equipo' : 'Tu espacio está conectado' : loading ? 'Conectando tu espacio…' : 'Sin conexión'}</strong><span>{data.sync ? 'Copia local cifrada' : 'SQL Server'}</span></div><Database size={16}/></div><button className="profile" onClick={() => setModal('help')}><span className="avatar">JD</span><span><strong>Mi espacio personal</strong><small>Hecho para pensar con claridad</small></span><CircleHelp size={16}/></button></div>
    </aside>
    <div className="main-shell"><header className="topbar"><div className="breadcrumb"><IconButton label="Abrir menú" className="icon-button mobile-menu" onClick={() => setMobile(!mobile)}><Menu size={20}/></IconButton><BookOpen size={16}/><span>Mi espacio</span><ChevronRight size={13}/><strong>{title}</strong></div><div className="global-search" role="search" aria-label="Buscar en mis notas"><Search size={20} aria-hidden="true"/><SaInput uppercase={false} ref={search} label="Buscar notas" hideLabel value={query} onValueChange={setQuery} placeholder="Buscar por título, contenido o etiqueta…" size="md"/>{query ? <IconButton label="Limpiar búsqueda" onClick={() => setQuery('')}><X size={14}/></IconButton> : <kbd>Ctrl K</kbd>}</div><button className="top-avatar" onClick={() => setModal('help')} aria-label="Ayuda de mi espacio">JD</button></header>
      <main className="main-content"><div className="page-heading"><div><div className="eyebrow"><span className="tiny-line"/>TU SEGUNDO CEREBRO, UN POCO MÁS ORDENADO</div><h1>{scope === 'all' ? 'Un lugar para tus ideas' : title}<span className="heading-dot">.</span></h1><p>{descriptions[scope]}</p></div><div className="heading-actions"><SaButton variant="secondary" size="sm" label="Importar / exportar" icon={<ArrowDownToLine size={16}/>} onClick={() => setModal('backup')} disabled={!connected}/><SaButton size="sm" label="Nueva nota" icon={<Plus size={17}/>} onClick={() => newNote()} disabled={!connected}/></div></div>
        {scope === 'solutions' ? (
          <SolutionsView
            ...
            onNewNote={draft => {
              ...
              setEditor({ ...empty, categoryId: cat, ...draft });
              setMobile(false);
            }}
          />
        ) : (
        ...
        {data.sync && <SyncStatus status={data.sync} onRefresh={refresh}/>}
    ...
    {modal === 'help' && <Modal title="Menos ruido. Más ideas." subtitle="Una pequeña guía para aprovechar tu espacio." onClose={() => setModal(null)}><div className="help-body"><div><Columns3/><h3>De una idea a un avance</h3><p>El tablero muestra Por hacer, En progreso, En validación y Completado. Arrastra tus notas entre columnas o cambia su estado desde la tarjeta o el editor. Elige Fuera del tablero para conservar una nota en la biblioteca sin mostrarla en el kanban.</p></div><div><FileText/><h3>Escribe como piensas</h3><p>Usa Markdown, código, etiquetas y listas de tareas. Pulsa Código otra vez o Texto simple para quitar el bloque sin perder el contenido. Usa Continuar debajo del código para seguir escribiendo fuera de él. Cada guardado crea una versión recuperable.</p></div><div><Database/><h3>Tu espacio es local</h3><p>Las notas, imágenes e historial se guardan cifrados en este equipo. Cada minuto, mientras Apuntes esté en ejecución, se intenta sincronizar con SQL Server. Puedes trabajar sin VPN después de completar la primera copia. Exporta un respaldo cifrado para llevar tus notas a otro lugar.</p></div><div className="shortcuts"><span>Buscar notas<kbd>Ctrl K</kbd></span><span>Nueva nota<kbd>Ctrl Alt N</kbd></span><span>Guardar nota<kbd>Ctrl Enter</kbd></span><span>Deshacer / rehacer contenido<kbd>Ctrl Z / Ctrl Shift Z</kbd></span><span>Alternar negrita<kbd>Ctrl B</kbd></span></div><p className="muted">La papelera conserva tus notas hasta que decidas restaurarlas. Archivar oculta una nota de las vistas activas.</p></div></Modal>}
