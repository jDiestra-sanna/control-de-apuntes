...
process.env.APUNTES_STORAGE = 'sql'; // Existing SQL contract suite; offline mode has a separate isolated suite.
...
try {
  ...
  await expect(dialog.getByLabel('Estado del kanban')).toHaveValue('inbox');
  await expect(dialog.getByText('Esta nota se guarda en la biblioteca y no aparece en el kanban.')).toBeVisible();
  ...
  await expect(page.locator('.board-column > .column-heading h3')).toHaveText(['Por hacer', 'En progreso', 'En validación', 'Completado']);
  await expect(page.locator('.library-title > span')).toHaveText('6');
  await expect(page.locator('.note-card')).toHaveCount(6);
  await expect(page.getByRole('heading', { name: 'Ideas que merecen un siguiente paso', exact: true })).toHaveCount(0);
  await page.getByRole('button', { name: 'Nueva nota', exact: true }).first().click();
  await expect(page.getByRole('dialog').getByLabel('Estado del kanban')).toHaveValue('todo');
  await page.getByRole('dialog').getByRole('button', { name: 'Cerrar ventana' }).click();
  await page.getByRole('button', { name: 'Nueva nota en En validación', exact: true }).click();
  await expect(page.getByRole('dialog').getByLabel('Estado del kanban')).toHaveValue('review');
  await page.getByRole('dialog').getByRole('button', { name: 'Cerrar ventana' }).click();
  ok('Kanban con cuatro columnas, contador correcto y nuevas notas en la columna elegida');
  await page.getByRole('textbox', { name: 'Buscar notas' }).fill('Nota de verificación');
  ...
  await page.locator('.note-card').dragTo(page.getByRole('region', { name: 'En validación', exact: true }));
  await expect.poll(async () => (await request('/workspace')).data.notes.find(n => n.id === saved.id).status).toBe('review');
  await page.reload();
  await page.getByRole('button', { name: /Tablero kanban/ }).click();
  await page.getByRole('textbox', { name: 'Buscar notas' }).fill('Nota de verificación');
  await expect(page.getByRole('region', { name: 'En validación', exact: true }).locator('.note-card')).toHaveCount(1);
  await page.getByLabel('Estado de Nota de verificación').selectOption('inbox');
  await expect(page.locator('.note-card')).toHaveCount(0);
  await expect(page.locator('.library-title > span')).toHaveText('0');
  await page.getByRole('button', { name: 'Vista de tarjetas', exact: true }).click();
  await expect(page.getByRole('heading', { name: 'Nota de verificación', exact: true })).toBeVisible();
  await page.getByRole('button', { name: 'Filtros', exact: true }).click();
  await page.getByLabel('Estado', { exact: true }).selectOption('inbox');
  await expect(page.locator('.note-card')).toHaveCount(1);
  await page.getByRole('button', { name: 'Vista kanban', exact: true }).click();
  await expect(page.getByLabel('Estado', { exact: true })).toHaveValue('');
  await expect(page.getByLabel('Estado', { exact: true }).locator('option')).toHaveText(['Todos los estados', 'Por hacer', 'En progreso', 'En validación', 'Completado']);
  await expect(page.locator('.note-card')).toHaveCount(0);
  await page.getByRole('button', { name: 'Vista de tarjetas', exact: true }).click();
  await page.getByRole('heading', { name: 'Nota de verificación', exact: true }).click();
  await page.getByRole('dialog').getByLabel('Estado del kanban').selectOption('review');
  await page.getByRole('dialog').getByRole('button', { name: 'Guardar nota', exact: true }).click();
  await expect(page.getByRole('dialog')).toHaveCount(0);
  await page.getByRole('button', { name: 'Vista kanban', exact: true }).click();
  await expect(page.getByRole('region', { name: 'En validación', exact: true }).locator('.note-card')).toHaveCount(1);
  await page.getByRole('button', { name: 'Filtros', exact: true }).click();
  ok('Fuera del tablero oculta sin eliminar; biblioteca, filtros y editor permiten recuperarla');
